...
class SubmitAnswersView(APIView):
    ...
    def post(self, request):
        ...
        answers_map = {
            str(a["id"]): a["answer"] for a in answers if isinstance(a, dict)
        }
            ...
            # ── DESCRIPTIVE ──
                ...
                try:
                    ...
                    if result.get("roadmap_directives"):
                        master_directives.append(
                            result.get("roadmap_directives")
                        )
                    ...
                    # AI outputs
                    mastered = normalize_list(
                        result.get("mastered_topics", [])
                    )
                    gaps = normalize_list(
                    ...
                    print(f"AI Eval failed Q{q.get('id')}: {e}")

        # ── SCORING ──
        final_score = (
            (total_mcq_correct / mcq_count) * 100
            if mcq_count > 0 else 0
        )

        avg_depth = (
            sum(depth_scores) / len(depth_scores)
            if depth_scores else 5
        )

        # ── CLEAN FINAL DATA (NO CRASH EVER) ──
        final_strengths = list(set(
            str(x) for x in strength_tags if x and not isinstance(x, dict)
        ))

        final_weaknesses = list(set(
            str(x) for x in weakness_tags if x and not isinstance(x, dict)
        ))
        ...
        has_full_progression = all(
            tier in completed_tiers
        ...
        if has_full_progression:
            ...
            master_gaps = []
            for s in all_swots:
                master_gaps.extend(s.weaknesses or [])
